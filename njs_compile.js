		...
		switch(node.type) {
		  ...
		  case NEW: case NEW_WITH_ARGS: case CALL:
			...
			var checkRetNode = noderize({
				...
				thenPart: noderize({
					type: SEMICOLON,
					expression: codeNode("return ~.fh", scopeResolver.getCurrentFrame())
				})
			});
			...
			// add our frame as an argument to the call
			node[1].push(codeNode(scopeResolver.getCurrentFrame()));				
			break;
		  ...
		  case COMMA: case ASSIGN: case CONDITIONAL: case OR: case AND:
		  ...
		  case DOT:
			// don't execute n[1] because it might resolve to a scoped var
		  	node[0] = execute(node[0]);
		  	break;
...
NjsRuntime.notifierMethods = {
	...
	wait: function() {
		var timeLimit, coroutine;
		if (arguments.length == 2) {
			timeLimit = arguments[0];
			coroutine = arguments[1];
		} else {
		...
		if (coroutine.isResuming())
			return coroutine.commence();
		...
		this.waiting.push(coroutine);
		if (timeLimit && NjsGlobal.setTimeout) {
			coroutine.timeout = NjsRuntime.getEpoch() + timeLimit;
...
NjsRuntime.resume = function(frame) {
	while(frame.pf != null) {
		// set the child frame of our parent so that resume() works
		frame.pf.cf = frame;		
		frame = frame.pf;
	}
...
NjsRuntime.enterFrame = function(frameThis, args /*, ... argNames ... */) {
	...
	if (frame == null || typeof(frame) != "object" || frame.constructor != NjsCoroutineFrame) {
		...
		frame = new NjsCoroutineFrame(null);
		...
		if (frame.state == this.CALL) {
			...
			frame = new NjsCoroutineFrame(frame);
			...
			// null out the child frame to be safe and not leak memory
			frame.pf.cf = null;
...
function NjsCoroutineFrame(parentFrame) {
	this.pf = parentFrame;
}
